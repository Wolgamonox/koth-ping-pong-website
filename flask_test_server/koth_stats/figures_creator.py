...
def generate_fig_summary(filename, as_bytes=False):
    ...
    player_colors = {
        name: color
        for name, color in zip(players, sns.color_palette("Pastel1", n_colors=len(players)))
    }

    crowns_claimed = sc.get_crowns_claimed(df, players)
    total_time_king = sc.get_total_time_king(df, players)
    reign_time = sc.get_reign_time(df, players)
    graph_vector = sc.get_crown_transition_graph(df, players, total_duration_seconds)
    ...
    sns.boxplot(reign_time, x="Name", y="Interval", palette=player_colors, ax=ax)
    ax.set_xlabel("")
