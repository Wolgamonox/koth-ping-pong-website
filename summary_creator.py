...
def generate_summary(filename):
    ...
    crowns_claimed = df.drop(index=0).groupby('Name').count().rename({'Interval': 'Claimed'}, axis=1).sort_values('Claimed', ascending=False)
    total_time_king = df.groupby('Name').sum().rename({'Interval': 'Duration'}, axis=1).sort_values('Duration', ascending=False)
    mean_reign_time = df.groupby('Name').mean().rename({'Interval': 'Duration'}, axis=1).sort_values('Duration', ascending=False)


    df['interval_perc'] = df['Interval'].apply(lambda x: int(np.ceil(x/total_duration_seconds * 100)))
    ...
    print('First king: %s | Last king: %s ' % (df.iloc[0]['Name'], df.iloc[-1]['Name']))
